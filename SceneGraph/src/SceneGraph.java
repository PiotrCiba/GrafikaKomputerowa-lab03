...
public class SceneGraph extends JPanel {
	...
	// 6 rotating n-gons in an array
	private TransformedObject[] rotatingPoly = new TransformedObject[6];
	//array of scaling values for the polygons
	private double[][] PolygonScales = {
			{1,1},
			{1,1},
			{0.66,0.66},
			{0.66,0.66},
	...
	//array of translate values for the polygons
	private double[][] PolygonTranslates = {
			{0.8516, -0.3100},
			{-0.8516, 0.3100},
			{-1.2529, 1.2804},
			{-2.3771, 1.6896},
	...
	// 3 rectangles in an array
	private TransformedObject[] Rectangles = new TransformedObject[3];
	//array of scaling values for the rectangles
	private double[][] RectScales = {
			{2, 0.1875},
			{1.32, 0.12375},
			{1, 0.09375}
	};
	//array of translate  values for the rectangles
	private double[][] RectTranslates = {
			{0,0},
			{-1.815,1.485},
			{1.375,1.5}
	};
	
	// 3 triangles in an array
	private TransformedObject[] Triangles = new TransformedObject[3];
	//array of scaling vales for the triangle
	private double[][] TrigScales = {
			{0.5,1.5},
			{0.33,0.99},
			{0.25,0.75}
	};

	//array of translate values for the triangles
	private double[][] TrigTranslates = {
			{0,-1.5},
			{-1.815,0.495},
			{1.375,0.75}
	};
	//array of colours for the triangles
	private int[][] TrigColors = {
			{50,0,245},
			{144,0,123},
			{5,134,40}
	};
	...
	private void createWorld() {
		...
		for(int i=0;i<6;i++) {
			rotatingPoly[i] = new TransformedObject(filledpolygon);
			rotatingPoly[i].setScale(PolygonScales[i][0], PolygonScales[i][1]);
			rotatingPoly[i].setTranslation(PolygonTranslates[i][0], PolygonTranslates[i][1]);
			rotatingPoly[i].setColor(Color.BLACK);
			world.add(rotatingPoly[i]);
		}
		for(int i=0;i<3;i++) {
			//adding rectangles
			Rectangles[i] = new TransformedObject(filledRect);
			Rectangles[i].setScale(RectScales[i][0], RectScales[i][1]);
			Rectangles[i].setTranslation(RectTranslates[i][0], RectTranslates[i][1]);
			Rectangles[i].setRotation(-20);
			Rectangles[i].setColor(Color.RED);
			world.add(Rectangles[i]);
			//adding triangles
			Triangles[i] = new TransformedObject(filledTriangle);
			Triangles[i].setScale(TrigScales[i][0],TrigScales[i][1]);
			Triangles[i].setTranslation(TrigTranslates[i][0], TrigTranslates[i][1]);
			Triangles[i].setColor(new Color(TrigColors[i][0],TrigColors[i][1],TrigColors[i][2]));
	...
	public void updateFrame() {
		...
		for(int i=0;i<6;i++)
			rotatingPoly[i].setRotation(frameNumber*0.75); // (DELETE THIS EXAMPLE)
